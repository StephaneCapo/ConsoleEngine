...
RGBPixel getBitmapRGB(BITMAPINFO* info,unsigned char* data, int px, int py)
{
...
unsigned char* LoadBMPImage(const char* filename, int* sx, int* sy)
{
	...
	loadedData = LoadDIBitmap(filename, &BitmapInfo);
	*sx = BitmapInfo->bmiHeader.biWidth;
	*sy = BitmapInfo->bmiHeader.biHeight;
